                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else//登录成功
                        {
                            ...
                            //登录成功，启动接收线程负责接收数据
                            // 这个线程只启动一次就行，因为这个处理函数里面会有堵塞，所以只启动一次就可以，可以处理不同用户的数据
        ...
        case 2://register业务
            {
                ...
                else
                {
                    ...
                    {
                        json responsejs = json::parse(buffer);//反序列化成json
...
void readTaskHandler(int clientfd)
{
    ...
    {
        char buffer[1024] = {0};
        int len = recv(clientfd,buffer,1024,0);//将接收的信息存到buffer
        if (-1 == len || 0 == len)
        {
            close(clientfd);
            exit(-1);
        }
        json js= json::parse(buffer);//转成json格式
...
void mainMenu(int clientfd)
{
    ...
    while (isMainMenuRuning)
    {
        ...
        string command;//存储命令
        int idx = commandbuf.find(":");//找到返回起始下标，找不到返回-1
        if (-1 == idx)
        {
            command = commandbuf;
        }
...
void chat(int clientfd,string str)
{
    ...
    string meg = str.substr(idx+1,str.size()-1);
